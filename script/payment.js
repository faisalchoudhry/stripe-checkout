Stripe.setPublishableKey('pk_test_HBUY6uUd8f24PGoEe66UAZtW');
...
function validateForm() {
    ...
    if (validCard == 1) {
        ...
        if (!validateName.test(customerLastName)) {
            $('#customerLastName').addClass('require');
            $('#errorCustomerLastName').text('Invalid Last Name');
            valid = false;
        } else {
            $('#customerLastName').removeClass('require');
            $('#errorCustomerLastName').text('');
            valid = true;
        }
...
$(document).ready(function () {
    // console.log("ready!");
    $('#totalAmnt').text('$14.95');
    $('input[name=price]').val('1495');
    $('input[name=total_amount]').val('1495');
    $('input[name=currency_code]').val('USD');
    $('input[name=item_details]').val('AboutLife-Combo');
    $('input[name=item_number]').val('AL-Combo1495');
    $('input[type=radio][name=fav_language]').change(function () {
        if (this.value == 'Combo') {
            // alert("Allot Thai Combo");
            $('input[name=price]').val('1495');
            $('input[name=total_amount]').val('1495');
            $('#totalAmnt').text('$14.95');
            $('input[name=currency_code]').val('USD');
            ...
            // alert("Transfer Thai eBook");
            $('input[name=price]').val('995');
            $('input[name=total_amount]').val('995');
            $('#totalAmnt').text('$9.95');
            $('input[name=currency_code]').val('USD')
            $('input[name=item_details]').val('AboutLife-eBook')
            $('input[name=item_number]').val('AL-eBook995')
        } else if (this.value == 'Audiobook') {
            // alert("Transfer Thai Audiobook");
            $('input[name=price]').val('895');
            $('input[name=total_amount]').val('895');
            $('#totalAmnt').text('$8.95');
            $('input[name=currency_code]').val('USD');
            $('input[name=item_details]').val('AboutLife-Audiobook');
            $('input[name=item_number]').val('AL-Audiobook895');
        }
    });
});